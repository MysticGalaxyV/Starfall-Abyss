...
class LeaderboardView(View):
    def __init__(self, data_manager: DataManager, category: str = "level"):
    ...
    def add_category_select(self):
        ...
        options = [
            discord.SelectOption(label="Level", value="level", description="Sort by player level", default=self.category == "level"),
            discord.SelectOption(label="Gold", value="gold", description="Sort by gold amount", default=self.category == "gold"),
            discord.SelectOption(label="Battle Wins", value="wins", description="Sort by PvE battle wins", default=self.category == "wins"),
            discord.SelectOption(label="PvP Wins", value="pvp_wins", description="Sort by PvP battle wins", default=self.category == "pvp_wins"),
            discord.SelectOption(label="Dungeons Completed", value="dungeons_completed", description="Sort by dungeons completed", default=self.category == "dungeons_completed"),
            discord.SelectOption(label="Bosses Defeated", value="bosses_defeated", description="Sort by bosses defeated", default=self.category == "bosses_defeated")
        ]
        
        category_select = Select(placeholder="Select category", options=options)
        category_select.callback = self.category_callback
        self.add_item(category_select)
    
    def add_navigation_buttons(self):
        """Add prev/next page buttons"""
        prev_button = Button(label="⬅️ Previous", style=discord.ButtonStyle.secondary)
    ...
    def create_leaderboard_embed(self) -> discord.Embed:
        ...
        embed = discord.Embed(
            title=f"📊 {category_display.get(self.category, 'Level')} Leaderboard",
        ...
        embed.set_footer(text=f"Page {self.page + 1}/{max_pages} • Total Players: {total_players}")
        return embed


async def leaderboard_command(ctx, data_manager: DataManager, category: str = "level"):
