...
class TrainingOptionView(View):
    ...
    async def option_callback(self, interaction: discord.Interaction):
        ...
        if self.player_data.last_train and (now - self.player_data.last_train).total_seconds() < (cooldown_minutes * 60):
            remaining = cooldown_minutes * 60 - int((now - self.player_data.last_train).total_seconds())
        ...
        # Calculate exp gain
        exp_gain = random.randint(option_data['min_exp'], option_data['max_exp'])
        
        # Apply training results
        # Attribute points directly modify allocated stats
        self.player_data.allocated_stats[option_data['attribute']] += attribute_gain
...
class SkillPointsView(View):
    ...
    async def stat_callback(self, interaction: discord.Interaction):
        ...
        for item in self.children:
            ...
            await interaction.response.send_message("❌ Error: Stat not found!", ephemeral=True)
            return
        
        # Check if player has skill points
        if self.player_data.skill_points <= 0:
            await interaction.response.send_message("❌ You don't have any skill points to allocate!", ephemeral=True)
            return
    ...
    async def reset_callback(self, interaction: discord.Interaction):
        ...
        if total_allocated == 0:
            await interaction.response.send_message("❌ You don't have any allocated skill points to reset!", ephemeral=True)
            return
        ...
        async def confirm_callback(confirm_interaction):
            ...
            other_points = sum([v for k, v in self.player_data.allocated_stats.items() if k != "hp"])
            ...
            await confirm_interaction.response.edit_message(
                content=f"✅ Reset complete! Refunded {refund_points} skill points.",
                embed=embed,
                view=self
            )
        ...
        await interaction.response.edit_message(
            content="⚠️ Are you sure you want to reset all allocated skill points? This will refund approximately half of your invested points.",
            view=confirm_view
        )
    ...
    def create_skills_embed(self):
        ...
        embed = discord.Embed(
            ...
            description=f"Character: {self.player_data.class_name} (Level {self.player_data.class_level})\n"
        ...
        embed.add_field(
            name="🔢 Current Stats",
            value=f"**Power:** {base_stats['power']} (Base: {class_data['stats']['power']} + Allocated: {self.player_data.allocated_stats.get('power', 0)})\n"
...
async def train_command(ctx, data_manager: DataManager):
    ...
    if player_data.last_train and (now - player_data.last_train).total_seconds() < (cooldown_minutes * 60):
        remaining = cooldown_minutes * 60 - int((now - player_data.last_train).total_seconds())
        minutes, seconds = divmod(remaining, 60)
        await ctx.send(f"❌ You're still tired from your last training session! Rest for {minutes}m {seconds}s more.")
        return
    
    # Create training embed
    embed = discord.Embed(
        title="🏋️ Training Grounds",
        description=f"Welcome to the training grounds, {ctx.author.display_name}!\n"
...
async def skills_command(ctx, data_manager: DataManager):
    ...
    if player_data.skill_points <= 0:
        await ctx.send("❌ You don't have any skill points to allocate! Level up to earn more points.")
        return
