...
class ClassChangeView(View):
    ...
    def __init__(self, player_data: PlayerData, data_manager: DataManager):
        ...
        self.class_select = Select(placeholder="Select a class to change to",
                                   min_values=1,
                                   max_values=1)
        ...
        for class_name, requirements in available_classes.items():
            ...
            self.class_select.add_option(
                label=class_name,
                description=
                f"{GAME_CLASSES[class_name]['role']} - {requirements['status']}",
                value=class_name,
    ...
    def get_available_classes(self) -> Dict[str, Dict[str, Any]]:
        ...
        for class_name in STARTER_CLASSES.keys():
            if class_name in self.player_data.unlocked_classes:
                result[class_name] = {"available": True, "status": "Unlocked"}
            else:
                result[class_name] = {
                    "available": False,
        ...
        for class_name, class_data in ADVANCED_CLASSES.items():
            ...
            result[class_name] = {
                "available": class_name in self.player_data.unlocked_classes
                or available,
                "status": status
            }
    ...
    async def class_select_callback(self, interaction: discord.Interaction):
        ...
        if selected_class not in self.player_data.unlocked_classes:
            ...
            confirm_btn = Button(label=f"Unlock {selected_class}",
            ...
            async def confirm_callback(confirm_interaction):
                ...
                embed = discord.Embed(
                    title="🔄 Class Changed!",
                    description=
                    f"You have unlocked and changed to the **{selected_class}** class!",
                ...
                await confirm_interaction.response.edit_message(content=None,
                                                                embed=embed,
                                                                view=None)
            ...
            async def cancel_callback(cancel_interaction):
                await cancel_interaction.response.edit_message(
                    content="❌ Class change cancelled.", embed=None, view=None)
            ...
            confirm_btn = Button(label=f"Change to {selected_class}",
                                 style=discord.ButtonStyle.green,
                                 emoji="✅")
            ...
            async def confirm_callback(confirm_interaction):
                ...
                embed = discord.Embed(
                    title="🔄 Class Changed!",
                    description=
                    f"You have changed from **{old_class}** to **{selected_class}**!",
                    color=discord.Color.green())
    ...
    async def cancel_callback(self, interaction: discord.Interaction):
        """Handle cancellation"""
        await interaction.response.send_message("❌ Class change cancelled.",
                                                ephemeral=True)
        self.stop()
...
async def class_change_command(ctx, data_manager: DataManager):
    ...
    unlocked = "\n".join([f"• {cls}" for cls in player_data.unlocked_classes
                          ]) if player_data.unlocked_classes else "None"
    embed.add_field(name="Unlocked Classes", value=unlocked, inline=False)
